...
def new_transaction():
    ...
    # Emit a transaction event to all connected clients via Socket.IO
    socketio.emit('new_transaction', values, broadcast=True)
...
@socketio.on('capture_data_request')
def handle_capture_data():
    ...
    # Emit sensor data to the client that requested it
    emit('sensor_data', {'message': 'Sensor data captured', 'data': sensor_data})
